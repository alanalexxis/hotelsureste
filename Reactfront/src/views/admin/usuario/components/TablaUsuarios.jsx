...
const ColumnsTable = (props) => {
  ...
  const currentProducts = usuarios.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  React.useEffect(() => {
    const totalPages = Math.max(Math.ceil(usuarios.length / recordsPerPage), 1);

    if (currentPage > totalPages) {
      setCurrentPage(totalPages);
      window.location.hash = `#${currentPage - 1}`;
    }
  }, [usuarios, recordsPerPage, currentPage, setCurrentPage]);
    ...
    <div className="relative">
      <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
        ...
        <div className="mt-8 overflow-x-scroll ">
          <table className="w-full">
            ...
            <tbody>
              {currentProducts.map((usuario, index) => (
                <tr>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <button
          ...
          disabled={
            usuarios.length === 0 ||
            currentPage === Math.ceil(usuarios.length / recordsPerPage)
          }
          className={`flex items-center gap-x-2 rounded-md border px-5 py-2 text-sm capitalize transition-colors duration-200 ${
            currentPage === Math.ceil(usuarios.length / recordsPerPage) ||
            usuarios.length === 0
