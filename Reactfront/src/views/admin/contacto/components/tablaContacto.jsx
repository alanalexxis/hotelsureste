...
import privateKeyPath from "./private-key.pem";
          ...
          <tbody>
            {usuarios.map((usuario) => (
              <tr>
                ...
                <td className="text-sm font-bold text-navy-700 dark:text-white">
                  {visiblePasswords.includes(usuario.idmensajes) ? (
                    <span>
                      {" "}
                      {privateKeyPem
                        ? decryptMessage(usuario.mensaje)
                        : "Private key loading..."}
                    </span> // Mostrar la contraseña si el id de usuario está en visiblePasswords
                  ) : (
                    <FaEyeSlash
                      onClick={() => handleTogglePassword(usuario.idmensajes)}
